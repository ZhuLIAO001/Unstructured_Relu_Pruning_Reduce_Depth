...
model= torch.load('/home/ipp-9236/PYZhu/ICIP23/Resnet18_cifar10/baseEntropyExpoMagni_prun/model/sparsity_0.5').to(device)
...
# 	torch.save(temp_model, '/home/ipp-9236/PYZhu/ICIP23/Resnet18_cifar10/baseEntropy_prun_mind2/model/'+ name_model)
...
for i in range(1, 10):
	...
	for name, module in model.named_modules():
		if name in layers_to_prune:
			if torch.numel(torch.abs(module.weight)[module.weight != 0])==0:
				...
				# print(name)
				# if torch.isnan(module.weight[module.weight!=0]).any():
				# 	print(name)
				
			# print('mean',torch.mean(torch.abs(module.weight[module.weight!=0])))
	...
	while True:
		...
		for name, module in model.named_modules():
			if name in wait_distri_paras:
				...
				# if fix_prun_amount[name].item() == 'nan':
				# if torch.isnan(fix_prun_amount[name]):					
	...
	for name, module in model.named_modules():
		if name in layers_to_prune:
			...
			# print(name)
			# print(int(fix_prun_amount[name]))
	...
	for epoch in range(1,epochs+1):
		...
		torch.save(temp_model, '/home/ipp-9236/PYZhu/ICIP23/Resnet18_cifar10/baseEntropyExpoMagni_prun/model/'+ name_model)
	...
	plt.savefig('/home/ipp-9236/PYZhu/ICIP23/Resnet18_cifar10/baseEntropyExpoMagni_prun/Tradeoff_curve/'+'sparsity_acc_Tradeoff_curve_'+str(sparsity) + '.png')
	# plt.show()
